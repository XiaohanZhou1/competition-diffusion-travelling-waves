function gamma_over_D_powerlaw_fit()
...
%         for the fixed small-positive-speed branch.
...
%   This script targets the fixed small speed c = 0.01.
    ...
    D_list = logspace(log10(0.1), log10(15), 46);
...
function run_overall_threshold_figures(D_list, ~, opts)
    ...
    if nnz(largeFitMask) >= 2
        Dlarge = Dv(largeFitMask);
        gLarge = gv(largeFitMask);
        pLargePower = polyfit(log(Dlarge), log(gLarge), 1);
        alphaLarge = pLargePower(1);
        KLarge = exp(pLargePower(2));
        relLarge = norm(gLarge - KLarge*Dlarge.^alphaLarge) / norm(gLarge);

        fprintf('\n===== LARGE-D POWER-LAW FIT =====\n');
    ...
    if numel(D_small_valid) >= 2
        pSmallPower = polyfit(log(D_small_valid), log(gamma_small_valid), 1);
        alphaSmall = pSmallPower(1);
        KSmall = exp(pSmallPower(2));
        relSmall = norm(gamma_small_valid - ...
            KSmall*D_small_valid.^alphaSmall) / norm(gamma_small_valid);
    ...
    %% Small- and large-D power-law fits on common log-log axes
    ...
    DtruePlot = logspace(log10(min(Dtrue)), log10(max(Dtrue)), 1000);
    gammaTruePlot = exp(makima(log(Dtrue), log(gammaTrue), log(DtruePlot)));
    hNumerical = loglog(ax, DtruePlot, gammaTruePlot, 'k-', ...
        'LineWidth', 1.7);

    DplotSmall = logspace(log10(0.0101), log10(0.05), 300);
    hSmall = loglog(ax, DplotSmall, ...
        KSmall*DplotSmall.^alphaSmall, 'r--', 'LineWidth', 2.0);

    DplotLarge = logspace(log10(0.03), log10(14.999), 500);
    hLarge = loglog(ax, DplotLarge, ...
        KLarge*DplotLarge.^alphaLarge, ...
        'b--', 'LineWidth', 2.2);
    ...
    legend(ax, [hNumerical, hSmall, hLarge], ...
        {'Numerical branch ($c=0.01$)', ...
         sprintf('Fit: $%.3gD^{%.4f}$', KSmall, alphaSmall), ...
         sprintf('Fit: $%.3gD^{%.4f}$', KLarge, alphaLarge)}, ...
        'Location', 'best', 'Interpreter', 'latex');
    grid(ax, 'on');
    box(ax, 'on');
    set(ax, 'FontSize', 12, 'TickLabelInterpreter', 'latex', ...
        'XScale', 'log', 'YScale', 'log', 'GridAlpha', 0.25);
    ...
    loglog(axInset, DtruePlot(maskNumericalInset), ...
        gammaTruePlot(maskNumericalInset), 'k-', 'LineWidth', 1.4);
    maskSmallInset = DplotSmall <= 0.15;
    loglog(axInset, DplotSmall(maskSmallInset), ...
        KSmall*DplotSmall(maskSmallInset).^alphaSmall, ...
        'r--', 'LineWidth', 1.5);
    maskLargeInset = DplotLarge <= 0.15;
    loglog(axInset, DplotLarge(maskLargeInset), ...
        KLarge*DplotLarge(maskLargeInset).^alphaLarge, ...
        'b--', 'LineWidth', 1.5);
    xlim(axInset, [0.01 0.15]);
    ylim(axInset, [0.1 0.9]);
    ...
    if isfinite(KSmall)
        fprintf('Small-D fit: gamma_crit(D) ~= %.8g D^{%.8g}\n', ...
            KSmall, alphaSmall);
    end
    if isfinite(KLarge)
        fprintf('Large-D fit: gamma_crit(D) ~= %.8g D^{%.8g}\n', ...
            KLarge, alphaLarge);
    else
        fprintf('Not enough finite large-D threshold points for a power-law fit.\n');
    end
