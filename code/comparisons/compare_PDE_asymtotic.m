function compare_PDE_asymtotic()
% Solve
...
%   collapses to numerical noise.
    ...
    if cmin_plot <= 0 && cmax_plot >= 0
        hZero = contour(DD, GG, c_plot, [0 0], 'k', 'LineWidth', 2.0);
    end

    hCross = gobjects(0);
    crossover_field = c_plot - sqrt(DD);
    cross_min = min(crossover_field(:), [], 'omitnan');
    cross_max = max(crossover_field(:), [], 'omitnan');
    if cross_min <= 0 && cross_max >= 0
        hCross = contour(DD, GG, crossover_field, [0 0], ...
            'w--', 'LineWidth', 2.0);
    end
    ...
    hAsym = gobjects(0);
    if 1 - 2*a > 0
        D_asym = linspace(min(D_list), max(D_list), 500);
        gamma_asym = sqrt(2*D_asym/(1 - 2*a));
        keep_asym = gamma_asym >= min(gamma_list) & ...
            gamma_asym <= max(gamma_list);
        hAsym = plot(D_asym(keep_asym), gamma_asym(keep_asym), ...
            'r--', 'LineWidth', 2.0);
    else
        warning('The asymptotic curve is not real because 1-2a <= 0.');
    end

    legend_handles = gobjects(0);
    legend_labels = {};
    if ~isempty(hZero)
        legend_handles(end+1) = hZero(1); %#ok<AGROW>
        legend_labels{end+1} = '$c(D,\gamma)=0$'; %#ok<AGROW>
    end
    if ~isempty(hAsym)
        legend_handles(end+1) = hAsym; %#ok<AGROW>
        legend_labels{end+1} = '$\gamma=\sqrt{2D/(1-2a)}$'; %#ok<AGROW>
    end
    if ~isempty(hCross)
        legend_handles(end+1) = hCross(1); %#ok<AGROW>
        legend_labels{end+1} = 'Stefan-flux crossover'; %#ok<AGROW>
    end
