function plot_flux_smallc_asymptotics()
% plot_flux_smallc_asymptotics
%
% Figure A for the flux-type reduced model in the small-c regime.
%
% Compare the numerical phase-plane trajectory for
%
%     -W dW/dU = c W + R(U;a),    W(1)=0,
%
% with the small-c expansion
%
%     W(U) = W0(U) + c W1(U) + ...
%
% where
...
%
% For the flux-type model the interfacial condition is
%
%     W(0) = -sqrt(D)/(gamma sqrt(3)).
%
% Hence the flux parameter mu = sqrt(D)/gamma is obtained from
%
%     mu(c) = -sqrt(3) W(0;c).
    ...
    mu_num = -sqrt(3) * W_at_0;
    mu0 = sqrt(6 * integral(@(s) reaction(s, a), 0, 1, ...
        'RelTol', 1e-12, 'AbsTol', 1e-14));
    mu_two = -sqrt(3) * (endpoint_W0(a) + c * endpoint_W1(a));

    fprintf('Small-c flux-type phase-plane comparison\n');
    fprintf('  a = %.6g, c = %.6g\n', a, c);
    fprintf('  Numerical W(0) = %.8f\n', W_at_0);
    fprintf('  Numerical mu = sqrt(D)/gamma = -sqrt(3) W(0) = %.8f\n', mu_num);
    fprintf('  Zero-speed threshold mu0 = %.8f\n', mu0);
    fprintf('  Two-term small-c mu approximation = %.8f\n', mu_two);
    ...
    plot(axPhase, 0, -mu_num / sqrt(3), 'o', ...
        'Color', [0 0 1], 'MarkerFaceColor', [0 0 1], 'MarkerSize', 5);
...
function y = endpoint_W0(a)
    y = -sqrt(2 * integral(@(s) reaction(s, a), 0, 1, ...
        'RelTol', 1e-12, 'AbsTol', 1e-14));
end

function y = endpoint_W1(a)
    U = linspace(1e-6, 1 - 1e-6, 4000).';
    [W0, ~] = small_c_terms(U, a);
    y = integral(@(s) W0_interp(s, U, W0), 0, 1, ...
        'RelTol', 1e-10, 'AbsTol', 1e-12) / endpoint_W0(a);
end

function y = reaction(U, a)
    y = U .* (1 - U) .* (U - a);
end
