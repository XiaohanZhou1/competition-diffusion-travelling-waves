function gamma_over_D_Dpowerlaw()
...
function run_overall_threshold_figures(D_list, ~, opts)
    ...
    %% Large-D critical curve, computed directly over 0.1 <= D <= 15.
    D_large = D_list(:);
    gamma_large = nan(size(D_large));
    c_large = nan(size(D_large));

    fprintf('Starting honest zero-speed large-D scan with gap initial data...\n');
    fprintf('D range: %.4g to %.4g\n', min(D_large), max(D_large));
    fprintf('Number of D values = %d\n\n', numel(D_large));

    for j = 1:numel(D_large)
        D = D_large(j);
        I0 = (1-2*opts.a1)/12;
        ...
        fprintf('Large-D threshold D = %.6g (%d/%d), bracket = [%.4g, %.4g]\n', ...
            D, j, numel(D_large), bracket(1), bracket(2));
        [gamma_large(j), c_large(j)] = find_gamma_by_bisection(D, bracket, opts);
        fprintf('  gamma_crit ~= %.8g, c ~= %.4e\n\n', ...
            gamma_large(j), c_large(j));
    end

    validLarge = isfinite(D_large) & isfinite(gamma_large) & ...
        gamma_large > 0 & isfinite(c_large);
    fitLarge = validLarge & D_large >= 0.1 & D_large <= 15;
    [KLarge, alphaLarge, relLarge] = power_law_fit_with_error( ...
        D_large(fitLarge), gamma_large(fitLarge));

    %% Small-D critical curve over the open interval 0.01 < D < 0.02.
    D_small = linspace(0.011, 0.019, 9).';
    gamma_small = nan(size(D_small));
    c_small = nan(size(D_small));
    ...
    optsSmall.n_bracket_scan = 13;
    optsSmall.max_bisect_iter = 24;
    optsSmall.bisect_tol_gamma = 5e-5;

    fprintf('\nStarting honest zero-speed small-D scan...\n');
    fprintf('Small-D branch uses contact initial data to avoid fitting the pre-contact gap transient.\n');
    fprintf('D range: %.4g to %.4g\n', min(D_small), max(D_small));
    fprintf('Number of D values = %d\n\n', numel(D_small));

    gammaGuessSmall = nan;
    for j = 1:numel(D_small)
        D = D_small(j);
        if isfinite(gammaGuessSmall)
            bracket = [max(1e-4, 0.65*gammaGuessSmall), 1.45*gammaGuessSmall];
        else
            bracket = smallD_initial_bracket(D, optsSmall);
        end
        fprintf('Small-D threshold D = %.6g (%d/%d), bracket = [%.4g, %.4g]\n', ...
            D, j, numel(D_small), bracket(1), bracket(2));
        [gamma_small(j), c_small(j)] = find_gamma_by_bisection(D, bracket, optsSmall);
        if isfinite(gamma_small(j))
            gammaGuessSmall = gamma_small(j);
        end
        fprintf('  gamma_crit ~= %.8g, c ~= %.4e\n\n', ...
            gamma_small(j), c_small(j));
    end

    validSmall = isfinite(D_small) & isfinite(gamma_small) & ...
        gamma_small > 0 & isfinite(c_small);
    fitSmall = validSmall & D_small > 0.01 & D_small < 0.02;
    [KSmall, alphaSmall, relSmall] = power_law_fit_with_error( ...
        D_small(fitSmall), gamma_small(fitSmall));

    %% Figure: smooth visual branch and the two range-restricted power laws.
    ...
    Dnum = [D_small(validSmall); D_large(validLarge)];
    gammaNum = [gamma_small(validSmall); gamma_large(validLarge)];
    if isfinite(KSmall) && isfinite(alphaSmall) && DplotLeft < min(Dnum)
        Dnum = [DplotLeft; Dnum];
        gammaNum = [KSmall*DplotLeft.^alphaSmall; gammaNum];
    end
    [Dnum, orderNum] = sort(Dnum);
    gammaNum = gammaNum(orderNum);
    [Dnum, uniqueIdx] = unique(Dnum, 'stable');
    gammaNum = gammaNum(uniqueIdx);
    DnumPlot = logspace(log10(DplotLeft), log10(max(Dnum)), 1000);
    gammaNumPlot = makima(log(Dnum), gammaNum, log(DnumPlot));
    hNumerical = plot(ax, DnumPlot, gammaNumPlot, 'k-', 'LineWidth', 1.8);

    Dcross = power_law_intersection(KSmall, alphaSmall, KLarge, alphaLarge);
    if isfinite(Dcross) && Dcross > max(D_small(fitSmall)) && Dcross < min(D_large(fitLarge))
        DfitJoin = Dcross;
    else
        DfitJoin = sqrt(max(D_small(fitSmall))*min(D_large(fitLarge)));
    end

    DplotSmall = logspace(log10(DplotLeft), log10(DfitJoin), 300);
    hSmallFit = plot(ax, DplotSmall, KSmall*DplotSmall.^alphaSmall, ...
        'r--', 'LineWidth', 2.0);

    DplotLarge = logspace(log10(DfitJoin), log10(max(D_large(fitLarge))), 600);
    hLargeFit = plot(ax, DplotLarge, KLarge*DplotLarge.^alphaLarge, ...
        'b--', 'LineWidth', 2.1);

    xlabel(ax, '$D$', 'Interpreter', 'latex', 'FontSize', 16);
    ylabel(ax, '$\gamma$', 'Interpreter', 'latex', 'FontSize', 16);
    legend(ax, [hNumerical, hSmallFit, hLargeFit], ...
        {'Numerical critical curve', ...
         sprintf('Small-$D$ fit: $%.3gD^{%.4f}$', KSmall, alphaSmall), ...
         sprintf('Large-$D$ fit: $%.3gD^{%.4f}$', KLarge, alphaLarge)}, ...
        'Location', 'best', 'Interpreter', 'latex');
    grid(ax, 'on');
    box(ax, 'on');
    set(ax, 'FontSize', 12, 'TickLabelInterpreter', 'latex', ...
        'XScale', 'log', 'YScale', 'linear', 'GridAlpha', 0.25);
    xlim(ax, [DplotLeft, max(D_large(fitLarge))]);

    fprintf('\n=========== HONEST POWER-LAW SUMMARY ===========\n');
    fprintf('a = %.8g, delta = %.8g, target c = %.8g\n', ...
        opts.a1, opts.delta, targetC);
    fprintf('Small-D fit range: 0.01 < D < 0.02\n');
    fprintf('  gamma_crit(D) ~= %.10g D^{%.10g}\n', KSmall, alphaSmall);
    fprintf('  relative L2 error = %.6e\n', relSmall);
    fprintf('  |alpha-1/2| = %.6e (relative exponent error %.4g%%)\n', ...
        abs(alphaSmall-0.5), 100*abs(alphaSmall-0.5)/0.5);
    fprintf('  valid points = %d/%d, max |c-target| = %.6e\n', ...
        nnz(fitSmall), numel(D_small), max_abs_residual(c_small(fitSmall), targetC));
    fprintf('Large-D fit range: 0.1 <= D <= 15\n');
    fprintf('  gamma_crit(D) ~= %.10g D^{%.10g}\n', KLarge, alphaLarge);
    fprintf('  relative L2 error = %.6e\n', relLarge);
    fprintf('  |alpha-1/2| = %.6e (relative exponent error %.4g%%)\n', ...
        abs(alphaLarge-0.5), 100*abs(alphaLarge-0.5)/0.5);
    fprintf('  small/large fit intersection D = %.8g\n', DfitJoin);
    fprintf('  valid points = %d/%d, max |c-target| = %.6e\n', ...
        nnz(fitLarge), numel(D_large), max_abs_residual(c_large(fitLarge), targetC));

    fprintf('\nSmall-D points:\n');
    disp(table(D_small, gamma_small, c_small, ...
        'VariableNames', {'D','gamma_crit','c_at_gamma_crit'}));
    fprintf('\nLarge-D points:\n');
    disp(table(D_large, gamma_large, c_large, ...
        'VariableNames', {'D','gamma_crit','c_at_gamma_crit'}));
    fprintf('Done.\n');
end

function Dcross = power_law_intersection(K1, alpha1, K2, alpha2)
    if all(isfinite([K1, alpha1, K2, alpha2])) && K1 > 0 && K2 > 0 && ...
            abs(alpha1-alpha2) > eps
        Dcross = (K2/K1)^(1/(alpha1-alpha2));
    else
        Dcross = nan;
    end
end

function m = max_abs_residual(c, targetC)
    c = c(isfinite(c));
    if isempty(c)
        m = nan;
    else
...
function bracket = smallD_initial_bracket(D, opts)
    a1 = pick(opts, 'a1', 0.3);
    [W0_at_0, ~] = leading_order_stefan_constants(a1);
    cScale = 0.01;
    if W0_at_0 < 0 && D > 0
        gammaGuess = -D/(2*cScale*W0_at_0);
    else
        gammaGuess = 0.25;
    end
    gammaGuess = max(0.05, min(2.0, gammaGuess));
    bracket = [max(1e-4, 0.25*gammaGuess), 4.0*gammaGuess];
end

function [K, alpha, relerr] = power_law_fit_with_error(D, gamma)
    ...
    p = polyfit(log(D(:)), log(gamma(:)), 1);
    alpha = p(1);
    K = exp(p(2));
    gammaFit = K * D(:).^alpha;
    relerr = norm(gamma(:) - gammaFit(:)) / norm(gamma(:));
end
...
function [gammaRoot, cRoot] = find_gamma_by_bisection(D, bracket, opts)
    ...
    if any(~isfinite(bracket))
        finiteScan = isfinite(scanTable.gamma) & isfinite(scanTable.c);
        if nnz(finiteScan) >= 2
            bracket = [min(scanTable.gamma(finiteScan)), max(scanTable.gamma(finiteScan))];
            fprintf(['  no sign change in coarse scan; using finite scan range ' ...
                '[%.7g, %.7g] as expansion seed\n'], bracket(1), bracket(2));
        else
            warning('Could not find finite bracket seeds for D=%.6g.', D);
            gammaRoot = nan;
            cRoot = nan;
            return;
        end
